...
} catch (e) {} // Iframes where we inject translation can be non-text/html,
...
const isAllowedIframeContentType = ['text/html', 'application/pdf'].includes(document.contentType); // Do not run on non-web pages (file://), test pages, safari extension pages (i.e. safari prefs)
// or non-top Safari pages

const isWeb = window.location.protocol === "http:" || window.location.protocol === "https:";
const isTestPage = Zotero.isBrowserExt && window.location.href.startsWith(browser.runtime.getURL('test')); // Not scraping on hidden iframes and only select frames
...
Zotero.Inject = {
  async init() {
    ...
    await Zotero.initInject(); // Zotero namespace APIs now initialized
