...
function getParent(rows, id) {
  ...
  while (true) {
    pos--; // This shouldn't happen unless a root is missing
...
Zotero.UI.ProgressWindow = class ProgressWindow extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    document.addEventListener('visibilitychange', this.handleVisibilityChange.bind(this)); // Preload other disclosure triangle state
  ...
  componentDidUpdate() {
    ...
    if (this.focusHeadlineSelect) {
      ...
      if (this.headlineSelectNode.current) {
        ...
        this.focusTreeOnUpdate = false; // This is a hacky approach, but figuring out when the focused tree row was rendered
    ...
    window.requestAnimationFrame(() => {
      if (this.lastHeight != this.rootNode.scrollHeight // In Firefox this ends up being 0 when the pane closes
      && this.rootNode.scrollHeight != 0) {
        this.lastHeight = this.rootNode.scrollHeight;
        this.sendMessage('resized', {
          height: this.rootNode.scrollHeight
        });
      }
    });
  } //
  ...
  changeHeadline(text, target, targets) {
    ...
    if (targets) {
      ...
      this.expandToTarget(targets, target); // Record which rows are expanded so they're not collapsed in setFilter()
  ...
  addError() {
    this.setState(prevState => {
      ...
      let newError = Array.from(arguments); // Add error if new
  ...
  handleAlerts(items) {
    ...
    let toplevelItems = items.filter(item => !item.parentItem && item.percentage == 100); // Do not announce anything until all top level items are saved

    if (!this.announceAlerts || toplevelItems.length == 0) return; // Generate the first top level message "Saving X items ..."
    ...
    let parentItemIndex = 0; // Generate messages about attachments and notes
    ...
    this.alertTimeout = setTimeout(() => {
      let logNode = document.getElementById("messageLog"); // Add message that the dialog will close in the end
      ...
      for (let {
        ...
        if (logNode.querySelector(`div[value="${CSS.escape(text)}"]`)) continue; // Insert new log entry
  ...
  renderHeadlineSelect() {
    var rowTargets = [this.state.target, // Show recent targets
  ...
  setFilter() {
    ...
    let isFilterEmpty = filter.length == 0; // Show the cross icon only when the filter is non-empty
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      let row = rows[i];
      let isPassing = row.name.toLowerCase().includes(filter); // If a row passes the filter, record it
    ...
    this.setState(prevState => {
      ...
      if (isFilterEmpty && Object.keys(this.expandedRowsCache).length > 0) {
        ...
        this.expandedRowsCache = {}; // Ensure that the selected row's parent's are not collapsed 
  ...
  renderTargetSelector() {
    return this.state.targetSelectorShown ? React.createElement("div", null, React.createElement("div", {
  ...
  renderItem(item) {
    var itemStyle = Object.assign({}, // Start at 50% opacity
    {
    ...
    var iconStyle = Object.assign({}, // Indent child items
    {
...
class TargetTree extends React.Component {
  ...
  handleKeyPress(event) {
    if (!event.altKey && !event.ctrlKey && !event.metaKey) {
      ...
      } else {// TODO: Find-as-you-type navigation
      }
    }
  ...
  collapseCurrentLibrary() {
    ...
    var collapse = []; // First find the last row in the library

    while (pos + 1 < this.props.rows.length) {
      pos++;
      let current = this.props.rows[pos].id; // If we hit another library, go back one
    ...
    while (true) {
      ...
      collapse.push(current.id); // When we reach a library, select it and stop
  ...
  expandCurrentLibrary() {
    var focused = this.props.focused;
    var pos = this.props.rows.findIndex(row => row.id == focused.id);
    var expand = []; // First find the library row
    ...
    while (true) {
      ...
      let current = this.props.rows[pos]; // When we reach another library, select it and stop
